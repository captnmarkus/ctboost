...
test_that("vector predictor format 3 is rejected before scoring", {
  path <- tempfile(fileext = ".json")
  on.exit(unlink(path), add = TRUE)
  writeLines(
    '{"format":"ctboost-json-predictor","format_version":3,"multi_strategy":"multi_output_tree"}',
    path,
    useBytes = TRUE
  )
  expect_error(
    ctboost_load_predictor(path),
    "unsupported predictor format version: 3",
    fixed = TRUE
  )
})
