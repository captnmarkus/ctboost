...
reuse the existing transport. Parent fitting belongs to hpo_full_0160.py exclusively.
"""
...
def prepare_package(root, state, execution, slot, shard):
    kernel = (
        slot.get("kernel")
        or f"{state['owner']}/ctboost-0160-full25-{state['run_id']}-w{slot['slot']}"
    )
...
def validate_download(destination, shard, *, plan, execution, worker_hash):
    ...
    paths = list(destination.rglob("artifacts/manifest.json"))
    if len(paths) != 1:
        raise ValueError("Expected exactly one full-run bundle manifest")
    manifest_path = paths[0]
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if any(manifest.get(key) != value for key, value in identity.items()):
        raise ValueError("Full-run remote manifest identity differs")
    if (
    ...
    for record in records:
        ...
        if record["status"] == "complete":
            if (
                ...
                or not isinstance(child.get("outer_split"), dict)
            ):
