...
def _model_fingerprint(
    ...
    # Predictor format v2 makes the input representation part of model
    # identity. Keep the historical v1 fingerprint byte-for-byte compatible.
    if scoring_payload.get("format_version") == 2:
    ...
    *,
    ...
    manifest = {
        ...
        "model": {
            ...
            "iteration_count": len(scoring_payload["trees"]) // prediction_dimension,
            "prediction_dimension": prediction_dimension,
...
def validate_inference_manifest(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if type(document.get("schema_version")) is not int or (
        document.get("schema_version") != INFERENCE_MANIFEST_SCHEMA_VERSION
    ):
        raise ValueError(
    ...
    if tree_count % prediction_dimension != 0:
        raise ValueError(
    ...
    expected_iteration_count = tree_count // prediction_dimension
    if (
