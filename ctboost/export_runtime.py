...
from .export_payload import (
    JSON_PREDICTOR_FORMAT,
    JSON_PREDICTOR_FORMAT_VERSION,
    VECTOR_JSON_PREDICTOR_FORMAT_VERSION,
)
...
class ExportedPredictor:
    def __init__(self, payload: dict[str, Any]) -> None:
        ...
        if self.format_version >= JSON_PREDICTOR_FORMAT_VERSION and (
            ...
            raise ValueError("predictor format version 2 or newer requires an explicit format identifier")
        ...
        self.multi_strategy = self.payload.get("multi_strategy", "one_output_per_tree")
        ...
        if format_version == VECTOR_JSON_PREDICTOR_FORMAT_VERSION and not self._vector_leaves:
            raise ValueError("vector predictor format 3 requires multi_strategy='multi_output_tree'")
        if self._vector_leaves and (
            format_version != VECTOR_JSON_PREDICTOR_FORMAT_VERSION
            or self.prediction_dimension <= 1
        ):
            raise ValueError("vector predictor requires format_version 3 and multiple outputs")
        if self._vector_leaves and self.objective_name.strip().lower() not in {
            "multiclass", "softmax", "softmaxloss"
        }:
        ...
        if not self._vector_leaves and len(trees) % self.prediction_dimension != 0:
    ...
    def _validate_tree_model(self) -> None:
        trees_per_iteration = 1 if self._vector_leaves else self.prediction_dimension
        if len(self.trees) % trees_per_iteration != 0:
            raise ValueError("tree count must be divisible by prediction_dimension")
        iteration_count = len(self.trees) // trees_per_iteration
        if self._vector_leaves and self.tree_learning_rates and (
            len(self.tree_learning_rates) != iteration_count
        ):
            ...
            for node_index, node in enumerate(nodes):
                ...
                if self._vector_leaves:
                    ...
                    for weight in weights:
                        _require_finite_number(weight, f"vector {context}.leaf_weights")
                elif node.get("leaf_weights"):
    ...
    def _validate_input_envelope(self) -> None:
        ...
        if model.get("multi_strategy", "one_output_per_tree") != self.multi_strategy:
            raise ValueError("predictor manifest multi_strategy mismatch")
