...
VECTOR_MODEL_SCHEMA_VERSION = 3
SUPPORTED_MODEL_SCHEMA_VERSIONS = frozenset({1, MODEL_SCHEMA_VERSION, VECTOR_MODEL_SCHEMA_VERSION})
...
def _validate_model_document(document: Dict[str, Any]) -> None:
    ...
    state = document.get("booster_state")
    if document.get("artifact_type") == "ctboost.estimator":
        state = document.get("fitted_state", {}).get("booster_state")
    if isinstance(state, dict) and state.get("multi_strategy") == "multi_output_tree":
        if document.get("schema_version") != VECTOR_MODEL_SCHEMA_VERSION:
            raise ValueError("vector models require CTBoost model schema version 3")
